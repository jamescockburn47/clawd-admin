...
- **Qwen3.6-27B on `:8080` is ALSO the classifier/planner.** The separate 4B classifier on `:8085` was retired 2026-04-23 — running classification on the 27B gives materially better routing decisions, and a single-model topology is simpler to reason about. The `:8085` planner service + `:8081` 0.6B engagement classifier are both disabled. Classifier latency adds ~0.5–1 s to every message (acceptable per James's latency-vs-accuracy signal); keep-warm runs every tick to prevent cold-start.
- **Keywords remain the fallback when EVO is down.** Unchanged semantics.
- **EVO llama-server inventory (post-2026-04-23).** `:8080` Qwen3.6-27B-Q8_0 (chat + tools + coding + classification + planning — the ONE always-on model). `:8083` Qwen3-Embedding-8B (memory). `:8084` granite-docling (PDF/doc parsing). `:8086` gemma-4-31B (bot-council rollback, not Clint's concern). Retired services — do not re-enable without cause: `:8081` 0.6B classifier, `:8085` 4B planner, `llama-server-coder.service`, `llama-swap-{main,coder}.{service,timer}`.
