...
function backupTraceFile() {
  ...
  if (existsSync(INTERACTIONS_FILE)) {
    copyFileSync(INTERACTIONS_FILE, INTERACTIONS_BACKUP);
    interactionsBackupExists = true;
  }
...
function restoreTraceFile() {
  ...
  if (interactionsBackupExists && existsSync(INTERACTIONS_BACKUP)) {
    copyFileSync(INTERACTIONS_BACKUP, INTERACTIONS_FILE);
    unlinkSync(INTERACTIONS_BACKUP);
    interactionsBackupExists = false;
  } else if (!interactionsBackupExists && existsSync(INTERACTIONS_FILE)) {
    unlinkSync(INTERACTIONS_FILE);
  }
  if (feedbackBackupExists && existsSync(FEEDBACK_BACKUP)) {
    copyFileSync(FEEDBACK_BACKUP, FEEDBACK_FILE);
    unlinkSync(FEEDBACK_BACKUP);
    feedbackBackupExists = false;
